...
| 切换主题   | `SwitchThemeTaskUserControl.xaml` → `SwitchThemeTaskUserControlModel` → `SwitchThemeTask` / `AsstSwitchThemeTask`         | `Task/Interface/SwitchThemeTask.*`                                                          | `tasks/UiTheme/`；从候选主题名中随机选择并切换，空列表时跳过                   |
